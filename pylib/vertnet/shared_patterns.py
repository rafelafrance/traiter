...
def add(name: str, regexp: str) -> None:
    """Add a rule to SHARED."""
    SHARED[name] = fragment(name, regexp)


add('feet', r" foot s? | feet s? | ft s? (?! [,\w]) | (?<= \d ) ' ")
...
add('inches', ' ( inch e? s? | in s? ) (?! [a-dgi-km-ru-z] ) ')

add('metric_len', r' ( milli | centi )? meters? | ( [cm] [\s.]? m ) ')

add('len_units', '|'.join(
    [SHARED[x].pattern for x in ('metric_len', 'feet', 'inches')]))
LEN_UNITS = SHARED['len_units'].pattern

add('pounds', r' pounds? | lbs? ')
add('ounces', r' ounces? | ozs? ')
add('metric_mass', r"""
...
add('us_mass', '|'.join([SHARED[x].pattern for x in ('pounds', 'ounces')]))
...
add('sh_num', r""" \d+ ( \. \d+ )? | (?<= [^\d] ) \. \d+ """)
SH_NUM = SHARED['sh_num'].pattern

add('sh_val', f' ( {SH_NUM} | [?x]{{1,2}} | n/?d ) ')
...
add('ordinals', ' | '.join(ORDINALS))

# Time units
add('time_units', r'years? | months? | weeks? | days? | hours?')
...
add('dimension', r' (?P<dim> length | width ) ')

# Numeric sides interfere with number parsing so combine \w dimension
add('dim_side', fr""" {SHARED['dimension'].pattern} \s* (?P<side> [12] ) \b """)
