...
function formatParsedField(row, field) {
  ...
  row.parsed[field].forEach((trait, t) => {
    if (t) lines.push('<hr/>');
    if (Array.isArray(trait.value)) {
      const values = trait.value.join(', ');
      lines.push(`value: [${values}]`);
    } else {
    ...
    if (trait.side) lines.push(`side: ${trait.side}`);
    if (trait.includes) lines.push(`includes: ${trait.includes}`);
    if (trait.dimension) lines.push(`dimension: ${trait.dimension}`);
    if (trait.measured_from) lines.push(`measured_from: ${trait.measured_from}`);
    if (trait.as_is) lines.push('reported as is');
    if (trait.ambiguous_key) lines.push('has ambiguous key');
    if (trait.units_inferred) lines.push('units inferred');
    if (trait.estimated_value) lines.push('is an estimated value');
  });
