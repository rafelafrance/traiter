"""Get temp from various sources like CSV files or an SQLite database.

A vocabulary CSV file must contain the columns:
...
The only sqlite database currently supported is ITIS. Others may come later.
"""
...
class Terms:
    ...
    def pattern_dicts(self, columns: StrList) -> None:
        """Create a dict from a column in the temp."""
    ...
    def read_csv(self, csv_file: PathList, labels: Optional[StrList] = None) -> None:
        """Read and cache the temp from a CSV file.
    ...
    def hyphenate_terms(self) -> None:
        """Systematically handle hyphenated temp.

        We cannot depend on temp being present in a contiguous form. We need a
        systematic method for handling hyphenated temp. The hyphenate library is
        great for this but sometimes we need to handle non-standard hyphenations
        manually. Non-standard hyphenations are stored in the temp CSV file.
        """
        terms = []
        for term in self.terms:
    ...
    def drop(self, excludes: Union[str, List[str]], field: str = 'label') -> None:
        """Drop temp from the traits.

        If we include temp that interfere with patterns we can drop them.
        """
