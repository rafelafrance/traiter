...
**Hint:** Adding to the above hint you want to **leverage as much of spaCy as possible**. spaCy has a default pipeline. I may customize some steps and remove others, but I try to use what I can from spaCy. I use a modified tokenizer, the default Parts Of Speech (POS) model, and the default lemmatizer. A lemmatizer normalizes words to a canonical form, for instance “better” is lemmatized to “good”. Sometimes I will also use spaCy’s default NER model as input to my rules.

**Mistake:** A note of caution about the hint above. **Use models not of your making cautiously**. Most of what the spaCy team produces is rock solid but some models will change when there is a minor revision. A while back I was using a model on sentences to do a dependency parse on each one. Everything worked just fine until there was a minor spaCy upgrade and all the dependency parses changed ever so slightly and all of my rules broke. So far, the POS model has changed but not in any way that affects my rules.
...
Next I clean up any partial traits from above so that I can use the leftover words in other traits.
...
There are already well over 100 patterns for parsing taxa, trying to do all steps 2 through 4 in a single set of patterns would grow the numbers of patterns exponentially.
...
**Hint:** The traiter module is used by all the children traiter modules. However, not all the child traiter modules are up-to-date and some older ones may fail if you try to run their test. There is one old traiter module (VertNet) that does not even use spaCy. It’s that old. I’m waiting for it to be reused before I make any changes to it.
...
However, given the nature of what's being parsed, **some rule-based parsing** is both inevitable and practical. Using rules to bootstrap model training data is helpful. Additionally, I found that a hybrid parser that uses both a model and rules is a powerful parsing technique. When I used a custom vocabulary coupled with a simple NER model (ca. 2019), the results were nearly perfect. So good in fact, that I was able to use the model results to correct some rules. Unfortunately, that model was abandoned for non-technical reasons and I haven’t had time to revisit it since.
