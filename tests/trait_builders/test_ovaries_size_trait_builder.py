...
from pylib.trait_builders.ovaries_size_trait_builder \
    import OvariesSizeTraitBuilder


class TestOvariesSizeTraitBuilder(unittest.TestCase):

    parser = OvariesSizeTraitBuilder()

    def test_parse_01(self):
        self.assertEqual(
            self.parser.parse('ovaries = 8x5 mm'),
            [NumericTrait(
    ...
    def test_parse_02(self):
        self.assertEqual(
            self.parser.parse('ovary < 1 x 1 mm'),
            [NumericTrait(
    ...
    def test_parse_03(self):
        self.assertEqual(
            self.parser.parse('PIT tag #7F7D4D3A36 REMOVAL GRID #7 TRAP 1C'),
            [])
    ...
    def test_parse_06(self):
        self.assertEqual(
            self.parser.parse('Rt Ovary 2.0x3.5mm, Lft Ovary 2.1x4.0mm.'),
            [
                NumericTrait(
    ...
    def test_parse_07(self):
        self.assertEqual(
            self.parser.parse('ovaries: 20mm X 12mm, 18mm X 9mm.'),
            [NumericTrait(
