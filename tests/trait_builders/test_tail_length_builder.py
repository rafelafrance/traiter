...
class TestTailLengthTraitBuilder(unittest.TestCase):
    ...
    def test_parse_06(self):
        self.assertEqual(
            self.parser.parse('measurements:213-91-32-23'),
            [NumericTrait(
    ...
    def test_parse_07(self):
        self.assertEqual(
            self.parser.parse('213-91-32-23'),
            [NumericTrait(
                value=91, units='mm_shorthand', units_inferred=False,
    ...
    def test_parse_08(self):
        self.assertEqual(
            self.parser.parse('taillength=95;'),
            [NumericTrait(
                value=95, units=None, units_inferred=True, start=0, end=13)])

    def test_parse_09(self):
        self.assertEqual(
            self.parser.parse('reproductive data=testes abdominal; T = 3 x 1.8 ;'),
            [])
    ...
    def test_parse_13(self):
        self.assertEqual(
            self.parser.parse('ELEV G.T. 3900 FT'), [])
    ...
    def test_parse_15(self):
        self.assertEqual(
            self.parser.parse('95 on skull; [total: 773.5mm tail: 280.0 foot: 65.0 '
    ...
    def test_parse_16(self):
        self.assertEqual(
            self.parser.parse('Body and tail: 1690 mm; Body: 114000 g'),
            [])

    def test_parse_17(self):
        self.assertEqual(
            self.parser.parse('Other Measurements: nose-tail=60in., girth=39in.'),
            [])

    def test_parse_18(self):
        self.assertEqual(
            self.parser.parse('Imm. weight 50 kg, L. snout to tip of tail 1510,'),
            [])

    def test_parse_19(self):
        self.assertEqual(
            self.parser.parse('; trap identifier=CN02-T01/19 ;'),
            [])

    def test_parse_20(self):
        self.assertEqual(
            self.parser.parse('scrotal t.21mm'),
            [])
