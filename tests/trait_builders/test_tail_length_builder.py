...
from traiter.trait_builders.tail_length_trait_builder \
...
class TestTailLengthTraitBuilder(unittest.TestCase):
    ...
    def test_parse_01(self):
        self.assertEqual(
            ...
            [NumericTrait(value=102, units='mm', start=0, end=19)])

    def test_parse_02(self):
        self.assertEqual(
            PAR.parse('tail length=95 mm;'),
            [NumericTrait(value=95, units='mm', start=0, end=17)])

    def test_parse_03(self):
        self.assertEqual(
            PAR.parse('tail length=95;'),
            [NumericTrait(value=95, units_inferred=True, start=0, end=14)])

    def test_parse_04(self):
        self.assertEqual(
            PAR.parse(', "tail":"92", '),
            [NumericTrait(value=92, units_inferred=True, start=3, end=12)])

    def test_parse_05(self):
        self.assertEqual(
            PAR.parse('"tailLengthInMillimeters"="104",'),
            [NumericTrait(value=104, units='millimeters', start=1, end=30)])

    def test_parse_06(self):
        self.assertEqual(
            PAR.parse('measurements:213-91-32-23'),
            [NumericTrait(
                value=91, units='mm_shorthand', is_shorthand=True,
                start=0, end=25)])
    ...
    def test_parse_08(self):
        self.assertEqual(
            ...
            [NumericTrait(value=95, units_inferred=True, start=0, end=13)])
    ...
    def test_parse_15(self):
        self.assertEqual(
            ...
            [NumericTrait(value=85344, units='foot', start=29, end=45)])
