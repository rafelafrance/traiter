...
class TestTestesStateTraitBuilder(unittest.TestCase):
    ...
    parser = TestesStateTraitBuilder()
    ...
    def test_parse_02(self):
        self.assertEqual(
            self.parser.parse('testes descended'),
            [Trait(value='descended', start=0, end=16)])

    def test_parse_03(self):
        self.assertEqual(
            self.parser.parse('testes undescended'),
            [Trait(value='undescended', start=0, end=18)])

    def test_parse_04(self):
        self.assertEqual(
            self.parser.parse('testes undesc.'),
    ...
    def test_parse_17(self):
        self.assertEqual(
            self.parser.parse('tag# 1089; bag# 156; no gonads'),
    ...
    def test_parse_19(self):
        self.assertEqual(
            self.parser.parse('non-scrotal, sem. ves. 14 mm '),
    ...
    def test_parse_22(self):
        self.assertEqual(
            self.parser.parse('weight=75.6 g; reproductive data=Testes small'),
            [Trait(value='small', start=15, end=45)])

    def test_parse_23(self):
        self.assertEqual(
            self.parser.parse('weight=75.6 g; reproductive data=small'),
            [Trait(value='small', start=15, end=38)])

    def test_parse_24(self):
        self.assertEqual(
            self.parser.parse('puncture wound in left abdominal region.'),
            [])

    def test_parse_25(self):
        self.assertEqual(
            self.parser.parse(' reproductive data=plsc'),
            [])
    ...
    def test_parse_27(self):
        self.assertEqual(
            self.parser.parse('Mixed woods // TESTES NOT DESCENDED'),
            [Trait(value='not descended', start=15, end=35)])

    def test_parse_28(self):
        self.assertEqual(
            self.parser.parse('reproductive data=Uteri small, clear'),
            [])
    ...
    def test_parse_30(self):
        self.assertEqual(
            self.parser.parse('reproductive data=NS ;'),
            [Trait(value='ns', start=0, end=20)])

    def test_parse_31(self):
        self.assertEqual(
            self.parser.parse('reproductive data=SCR ;'),
            [Trait(value='scr', start=0, end=21)])

    def test_parse_32(self):
        self.assertEqual(
            self.parser.parse('; reproductive data=testes = 4x3 mm; '),
            [])
