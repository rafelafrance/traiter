...
from pylib.trait_builders.ear_length_trait_builder \
    import EarLengthTraitBuilder


class TestEarLengthTraitBuilder(unittest.TestCase):

    parser = EarLengthTraitBuilder()

    def test_parse_01(self):
        self.assertEqual(
            self.parser.parse('earLengthInmm: 9'),
            [NumericTrait(
                value=9, units='mm', units_inferred=False, start=0, end=16)])

    def test_parse_02(self):
        self.assertEqual(
            self.parser.parse('L. 9", T. 4", ear 9/16"'),
            [NumericTrait(
    ...
    def test_parse_03(self):
        self.assertEqual(
            self.parser.parse('L. 9", T. 4", HF. 2", E 1",'),
            [NumericTrait(
    ...
    def test_parse_05(self):
        self.assertEqual(
            self.parser.parse('{"earLength":"13", "gonadLength":"3"}'),
            [NumericTrait(
                value=13, units=None, units_inferred=True, start=2, end=16)])

    def test_parse_06(self):
        self.assertEqual(
            self.parser.parse('ear tag 570'),
            [])
    ...
    def test_parse_08(self):
        self.assertEqual(
            self.parser.parse('grid 9, station E1.'),
            [])

    def test_parse_09(self):
        self.assertEqual(
            self.parser.parse('ear from notch=17 mm;'),
            [NumericTrait(
    ...
    def test_parse_10(self):
        self.assertEqual(
            self.parser.parse('earfromcrown=17mm;'),
            [NumericTrait(
                value=17, units='mm', units_inferred=False,
    ...
    def test_parse_14(self):
        self.assertEqual(
            self.parser.parse('; ear from notch=.25 in'),
            [NumericTrait(
    ...
    def test_parse_16(self):
        self.assertEqual(
            self.parser.parse('"356142E 4805438N. Very small"'),
            [])

    def test_parse_17(self):
        self.assertEqual(
            self.parser.parse('Hind Foot: 19 EFN: 13 Weight: 16.3'),
            [NumericTrait(
    ...
    def test_parse_23(self):
        self.assertEqual(
            self.parser.parse('Cert #E0554185; Skull Seal #0406307'),
            [])

    def test_parse_24(self):
        self.assertEqual(
            self.parser.parse('T=12x8, E379700/N3740240'),
            [])

    def test_parse_25(self):
        self.assertEqual(
            self.parser.parse('Hawaiian chain.  Magnemite 610-E 7050.'),
            [])

    def test_parse_26(self):
        self.assertEqual(
            self.parser.parse('Gray, J. E. (1866).'),
            [])

    def test_parse_27(self):
        self.assertEqual(
            self.parser.parse('verbatim collector=C. H. Nellis 71-7-30-1'),
            [])
