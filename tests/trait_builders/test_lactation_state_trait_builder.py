...
class TestSexTraitBuilder(unittest.TestCase):
    ...
    parser = LactationStateTraitBuilder()

    def test_parse_01(self):
        self.assertEqual(
            self.parser.parse('lactating'),
    ...
    def test_parse_03(self):
        self.assertEqual(
            self.parser.parse('No embs; post lact.'),
            [Trait(value='post lact', start=9, end=18)])

    def test_parse_04(self):
        self.assertEqual(
            self.parser.parse('non-lactating'),
    ...
    def test_parse_06(self):
        self.assertEqual(
            self.parser.parse('recently lactating'),
            [Trait(value='recently lactating', start=0, end=18)])

    def test_parse_07(self):
        self.assertEqual(
            self.parser.parse('small mammaries, no lactation,'),
            [Trait(value='no lactation', start=17, end=29)])

    def test_parse_08(self):
        self.assertEqual(
            self.parser.parse('just finished lactating'),
            [Trait(value='just finished lactating', start=0, end=23)])

    def test_parse_09(self):
        self.assertEqual(
            self.parser.parse('reproductive data=non-lactating, non-pregnant'),
            [Trait(value='non-lactating', start=18, end=31)])

    def test_parse_10(self):
        self.assertEqual(
            self.parser.parse('Tail pencil; Not nursing, no embryos;'),
            [Trait(value='not nursing', start=13, end=24)])

    def test_parse_11(self):
        self.assertEqual(
            self.parser.parse('no. 8552; suckling'),
