...
from pylib.trait_builders.ovaries_state_trait_builder \
    import OvariesStateTraitBuilder


class TestOvariesStateTraitBuilder(unittest.TestCase):

    parser = OvariesStateTraitBuilder()
    ...
    def test_parse_04(self):
        self.assertEqual(
            self.parser.parse('; ovaries mod size;'),
    ...
    def test_parse_10(self):
        self.assertEqual(
            self.parser.parse(', ovaries immature (no lg folls) ;'),
    ...
    def test_parse_14(self):
        self.assertEqual(
            self.parser.parse('; both ovaries sev c.alb.;'),
    ...
    def test_parse_16(self):
        self.assertEqual(
            self.parser.parse('reproductive data=right ovary destroyed ;'),
            [Trait(value='destroyed', side='right', start=18, end=39)])

    def test_parse_17(self):
        self.assertEqual(
            self.parser.parse('reproductive data=large ovaries ;'),
            [Trait(value='large', start=18, end=31)])

    def test_parse_18(self):
        self.assertEqual(
            self.parser.parse('ovaries somewhat enlarged'),
            [Trait(value='somewhat enlarged', start=0, end=25)])

    def test_parse_19(self):
        self.assertEqual(
            self.parser.parse('ovaries imm.'),
            [Trait(value='imm', start=0, end=11)])

    def test_parse_20(self):
        self.assertEqual(
            self.parser.parse('ovaries: both w/sev c. alb;'),
            [Trait(value='w/sev c. alb', side='both', start=0, end=26)])

    def test_parse_21(self):
        self.assertEqual(
            self.parser.parse('corpus luteum visible in both ovaries'),
            [Trait(
