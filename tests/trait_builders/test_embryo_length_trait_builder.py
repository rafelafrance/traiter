...
from pylib.trait_builders.embryo_length_trait_builder \
    import EmbryoLengthTraitBuilder


class TestEmbryoCountTraitBuilder(unittest.TestCase):

    parser = EmbryoLengthTraitBuilder()

    def test_parse_01(self):
        self.assertEqual(
            self.parser.parse('crown-rump length=13 mm'),
            [NumericTrait(
                value=13, units='mm', units_inferred=False, start=0, end=23)])

    def test_parse_02(self):
        self.assertEqual(
            self.parser.parse('Embryo crown-rump length 22'),
            [NumericTrait(
    ...
    def test_parse_13(self):
        self.assertEqual(
            self.parser.parse('collector=CR 910025, E. E. Makela ;'),
            [])
    ...
    def test_parse_15(self):
        self.assertEqual(
            self.parser.parse('snap 47: middle Cow Cr.'),
            [])

    def test_parse_16(self):
        self.assertEqual(
            self.parser.parse('headwaters Kaluich Cr, ca 1500 ft'),
            [])
