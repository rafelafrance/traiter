...
class TestTestesSizeTraitBuilder(unittest.TestCase):
    ...
    def test_parse_13(self):
        self.assertEqual(
            ...
            [Trait(value=3, units_inferred=True, side='1', dimension='length',
                   ambiguous_key=True, start=1, end=21),
             Trait(value=2, units_inferred=True, side='2', dimension='length',
                   ambiguous_key=True, start=25, end=45)])

    def test_parse_14(self):
        self.assertEqual(
            PAR.parse('"gonadLengthInMM":"12", "gonadWidthInMM":"5",'),
            [Trait(value=12, units='mm', ambiguous_key=True,
                   dimension='length', start=1, end=21),
             Trait(value=5, units='mm', ambiguous_key=True, dimension='width',
                   start=25, end=43)])
    ...
    def test_parse_15(self):
        self.assertEqual(
            ...
            [Trait(value=9.1, units='mm', ambiguous_key=True, side='left',
                   dimension='width', start=0, end=23),
             Trait(value=9.2, units='mm', ambiguous_key=True, side='right',
                   dimension='width', start=25, end=49),
             Trait(value=16.1, units='mm', ambiguous_key=True, side='right',
                   dimension='length', start=51, end=77),
             Trait(value=16.2, units='mm', ambiguous_key=True, side='left',
                   dimension='length', start=79, end=104)])

    def test_parse_16(self):
        self.assertEqual(
            PAR.parse('"gonadLengthInMM":"9mm w.o./epid", '),
            [Trait(value=9, units='mm', ambiguous_key=True, dimension='length',
                   start=1, end=22)])
    ...
    def test_parse_18(self):
        self.assertEqual(
            ...
            [Trait(value=[10, 4], units_inferred=True, start=0, end=24)])
    ...
    def test_parse_20(self):
        self.assertEqual(
            ...
            [Trait(value=[9.0, 4.0], units_inferred=True, ambiguous_key=True,
                   start=8, end=13)])
    ...
    def test_parse_23(self):
        self.assertEqual(
            ...
            [Trait(value=[2, 4], units='mm', side='r',
                   start=0, end=28)])

    def test_parse_24(self):
        self.assertEqual(
            PAR.parse('reproductive data=T: L-2x4mm ; '),
            [Trait(value=[2, 4], units='mm', side='l',
                   start=0, end=28)])

    def test_parse_25(self):
        self.assertEqual(
            PAR.parse('testes (R) 6 x 1.5 & 5 x 2 mm'),
            [Trait(value=[6, 1.5], units_inferred=True, side='r',
                   start=0, end=18)])
    ...
    def test_parse_27(self):
        self.assertEqual(
            ...
            [Trait(value=[5, 3], units_inferred=True,
                   start=2, end=23)])
    ...
    def test_parse_31(self):
        self.assertEqual(
            ...
            [Trait(value=[20.0, 9.0], units="mm", side='right',
                   start=0, end=41)])

    def test_parse_32(self):
        self.assertEqual(
            PAR.parse('; reproductive data=Testes scrotal, 32x11'),
            [Trait(value=[32, 11], units_inferred=True, start=2, end=41)])
    ...
    def test_parse_35(self):
        self.assertEqual(
            ...
            [Trait(value=[20, 10], units='mm', side='r', start=2, end=40),
             Trait(value=[6, 4], units='mm', side='l', start=2, end=40)])

    def test_parse_36(self):
        self.assertEqual(
            PAR.parse('; reproductive data=R 20x10mm ;'),
            [Trait(value=[20, 10], units='mm', side='r', start=2, end=29)])
