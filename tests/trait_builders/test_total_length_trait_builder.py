...
from pylib.trait_builders.total_length_trait_builder \
    import TotalLengthTraitBuilder


class TestTotalLengthTraitBuilder(unittest.TestCase):

    parser = TotalLengthTraitBuilder()

    def test_parse_001(self):
        self.assertEqual(
            self.parser.parse('{"totalLengthInMM":"123" };'),
            [NumericTrait(
    ...
    def test_parse_006(self):
        self.assertEqual(
            self.parser.parse('"{"measurements":"308-190-45-20" }"'),
            [NumericTrait(
    ...
    def test_parse_007(self):
        self.assertEqual(
            self.parser.parse('308-190-45-20'),
            [NumericTrait(
                value=308, units='mm_shorthand', units_inferred=False,
    ...
    def test_parse_010(self):
        self.assertEqual(
            self.parser.parse('143-63-20-17=13'),
            [NumericTrait(
    ...
    def test_parse_011(self):
        self.assertEqual(
            self.parser.parse('** Body length =345 cm; Blubber=1 cm '),
            [NumericTrait(
    ...
    def test_parse_012(self):
        self.assertEqual(
            self.parser.parse('t.l.= 2 feet 3.1 - 4.5 inches '),
            [NumericTrait(
    ...
    def test_parse_013(self):
        self.assertEqual(
            self.parser.parse('2 ft. 3.1 - 4.5 in. '),
            [NumericTrait(
    ...
    def test_parse_014(self):
        self.assertEqual(
            self.parser.parse('total length= 2 ft.'),
            [NumericTrait(
    ...
    def test_parse_015(self):
        self.assertEqual(
            self.parser.parse('AJR-32   186-102-23-15  15.0g'),
            [NumericTrait(
    ...
    def test_parse_017(self):
        self.assertEqual(
            self.parser.parse('another; length=8 mm'),
            [NumericTrait(
                value=8, units='mm', units_inferred=False, ambiguous_key=True,
                start=9, end=20)])
    ...
    def test_parse_018(self):
        self.assertEqual(
            self.parser.parse('another; TL_120, noise'),
            [NumericTrait(
                value=120, units=None, units_inferred=True, start=9, end=15)])

    def test_parse_019(self):
        self.assertEqual(
            self.parser.parse('another; TL - 101.3mm, noise'),
            [NumericTrait(
    ...
    def test_parse_020(self):
        self.assertEqual(
            self.parser.parse('before; TL153, after'),
            [NumericTrait(
    ...
    def test_parse_023(self):
        self.assertEqual(
            self.parser.parse('Size=13 cm TL'),
            [NumericTrait(value=130, units='cm', units_inferred=False,
                          start=5, end=13)])

    def test_parse_024(self):
        self.assertEqual(
            self.parser.parse('det_comments:31.5-58.3inTL'),
            [NumericTrait(
    ...
    def test_parse_027(self):
        self.assertEqual(
            self.parser.parse('SVL 209 mm, total 272 mm, 4.4 g.'),
            [NumericTrait(
    ...
    def test_parse_030(self):
        self.assertEqual(
            self.parser.parse('TL (mm) 44,SL (mm) 38,Weight (g) 0.77 xx'),
            [NumericTrait(
    ...
    def test_parse_031(self):
        self.assertEqual(
            self.parser.parse('{"totalLengthInMM":"270-165-18-22-31", '),
            [NumericTrait(
    ...
    def test_parse_032(self):
        self.assertEqual(
            self.parser.parse('{"length":"20-29" }'),
            [NumericTrait(
    ...
    def test_parse_034(self):
        self.assertEqual(
            self.parser.parse('f age class: adult; standard length: 63-107mm'),
            [NumericTrait(
    ...
    def test_parse_035(self):
        self.assertEqual(
            self.parser.parse('Rehydrated in acetic acid 7/1978-8/1987.'),
            [])
    ...
    def test_parse_037(self):
        self.assertEqual(
            self.parser.parse('age class: adult; standard length: 18-21.5mm'),
            [NumericTrait(
                value=[18, 21.5], units='mm', units_inferred=False,
                start=18, end=44)])
    ...
    def test_parse_038(self):
        self.assertEqual(
            self.parser.parse('age class: adult; standard length: 18.0-21mm'),
            [NumericTrait(
    ...
    def test_parse_039(self):
        self.assertEqual(
            self.parser.parse('age class: adult; standard length: 18-21mm'),
            [NumericTrait(
                value=[18, 21], units='mm', units_inferred=False,
                start=18, end=42)])
    ...
    def test_parse_062(self):
        self.assertEqual(
            self.parser.parse('76 cm S.L., 4.7 kg'),
            [NumericTrait(
                value=760, units='cm', units_inferred=False, start=0, end=10)])

    def test_parse_063(self):
        self.assertEqual(
            self.parser.parse('set mark: 661 1-5 64-61'),
            [])

    def test_parse_064(self):
        self.assertEqual(
            self.parser.parse('{"totalLength":"970", "wing":"390" }'),
            [NumericTrait(
    ...
    def test_parse_068(self):
        self.assertEqual(
            self.parser.parse('Size=105 mm TL, 87.1 mm PCL'),
            [NumericTrait(
                value=105, units='mm', units_inferred=False, start=5, end=14)])

    def test_parse_069(self):
        self.assertEqual(
            self.parser.parse('Total Length: 185-252 mm'),
            [NumericTrait(
    ...
    def test_parse_071(self):
        self.assertEqual(
            self.parser.parse('"bottom":"rock?", "length":"278" }'),
            [NumericTrait(
    ...
    def test_parse_074(self):
        self.assertEqual(
            self.parser.parse('308-190-45-20-11-22'),
            [])

    def test_parse_075(self):
        self.assertEqual(
            self.parser.parse('10 12/07/1944'),
            [])

    def test_parse_076(self):
        self.assertEqual(
            self.parser.parse('12-07-1944'),
            [])

    def test_parse_077(self):
        self.assertEqual(
            self.parser.parse('{"measurements":"210-92-30" }'),
            [NumericTrait(
    ...
    def test_parse_078(self):
        self.assertEqual(
            self.parser.parse('measurements:210-92-30 308-190-45-20'),
            [NumericTrait(
                value=308, units='mm_shorthand', units_inferred=False,
    ...
    def test_parse_079(self):
        self.assertEqual(
            self.parser.parse('measurements:210-92-30 185-252 mm'),
            [NumericTrait(
                value=[185, 252], units='mm', units_inferred=False,
                start=0, end=33)])
    ...
    def test_parse_080(self):
        self.assertEqual(
            self.parser.parse('reproductive data=scars R2, L2 ;'),
            [])
    ...
    def test_parse_083(self):
        self.assertEqual(
            self.parser.parse('unformatted measurements=42-51 mm SL'),
            [NumericTrait(
    ...
    def test_parse_085(self):
        self.assertEqual(
            self.parser.parse('Cataloged by: R.L. Humphrey, 31 January 1995'),
            [])
    ...
    def test_parse_093(self):
        self.assertEqual(
            self.parser.parse('{"measurements":"TL=216.4 cm (+ 5 cm)" }'),
            [NumericTrait(
    ...
    def test_parse_094(self):
        self.assertEqual(
            self.parser.parse('t.l.= 2 feet, 4.5 inches '),
            [NumericTrait(
    ...
    def test_parse_095(self):
        target = ('The length reported (2560 cm = 85 feet) is a bit '
                  ...
                  'p. 24-25, 41-42')
        self.assertEqual(self.parser.parse(target), [])

    def test_parse_096(self):
        self.assertEqual(self.parser.parse('ELEV;1100 FT / 1500 FT?'), [])
    ...
    def test_parse_098(self):
        self.assertEqual(self.parser.parse('ELEV;7500 FT=6700 FT'), [])
    ...
    def test_parse_101(self):
        self.assertEqual(
            self.parser.parse('VERY YOUNG. LAST SPECIMEN CATALOGUED IN 1997.'),
            [])
    ...
    def test_parse_104(self):
        self.assertEqual(
            self.parser.parse('; trap identifier=SV01 S29/40 ;'),
            [])

    def test_parse_105(self):
        self.assertEqual(
            self.parser.parse('vagina opened; 4 embryos, R=3, L=1, CRL=28mm'),
            [])

    def test_parse_106(self):
        self.assertEqual(
            self.parser.parse('trap TL01  26g; 212-115-27-20=26g;'),
            [NumericTrait(
