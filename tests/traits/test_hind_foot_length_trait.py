...
class TestHindFootLengthTrait(unittest.TestCase):
    ...
    def test_parse_01(self):
        self.assertEqual(
            ...
            [Parse(value=30, units='mm', includes='claw',
                   start=0, end=25)])

    def test_parse_02(self):
        self.assertEqual(
            PAR.parse('"hindfootLengthInMM":"36"'),
            [Parse(value=36, units='mm', start=1, end=24)])

    def test_parse_03(self):
        self.assertEqual(
            PAR.parse('"hind foot length":"34.0"'),
            [Parse(value=34, units_inferred=True, start=1, end=24)])

    def test_parse_04(self):
        self.assertEqual(
            PAR.parse('; HindFoot: 30.0; '),
            [Parse(value=30, units_inferred=True, start=2, end=16)])
    ...
    def test_parse_07(self):
        self.assertEqual(
            ...
            [Parse(value=29, units_inferred=True, start=9, end=15)])

    def test_parse_08(self):
        self.assertEqual(
            PAR.parse('T-94mm, HF-30mm, E/n-19mm,'),
            [Parse(value=30, units='mm', start=8, end=15)])
    ...
    def test_parse_12(self):
        self.assertEqual(
            ...
            [Parse(value=31, units='millimeters', start=1, end=29)])
