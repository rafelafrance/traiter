...
class TestTotalLengthParser(unittest.TestCase):
    ...
    def test_search_and_normalize_22(self):
        self.assertDictEqual(
            ...
            {'haslength': 1,
             'lengthinmm': None,
             'lengthunitsinferred': None,
             'lengthtype': 'snout-vent length range'})
        ...
        self.assertDictEqual(
            TARGET.search_and_normalize(
                ['{"time collected":"1030", "water depth":"1-8", '
                 '"bottom":"abrupt lava cliff dropping off to sand at 45 ft.", '
                 '"length":"119-137" }']),
            {'haslength': 1,
             'lengthinmm': None,
             'lengthunitsinferred': None,
    ...
    def test_search_and_normalize_33(self):
        self.assertDictEqual(
            TARGET.search_and_normalize(
                ['field measurements on fresh dead specimen were 157-60-20-19-21g']),
            {'haslength': 1,
             'lengthinmm': 157,
             'lengthunitsinferred': 1,
             'lengthtype': 'total length'})
    ...
    def test_search_and_normalize_34(self):
        self.assertDictEqual(
            ...
            {'haslength': 1,
             'lengthinmm': None,
             'lengthunitsinferred': None,
             'lengthtype': 'standard length range'})
    ...
    def test_search_and_normalize65(self):
        self.assertDictEqual(
            TARGET.search_and_normalize(
                ['', 'SOURCE: M.A.CARRIKER JR.', 'LENGTH: 117MM. SOFT PARTS COLOR ON LABEL.']),
            {'haslength': 1,
             'lengthinmm': 117,
             'lengthunitsinferred': 0,
             'lengthtype': 'total length'})
